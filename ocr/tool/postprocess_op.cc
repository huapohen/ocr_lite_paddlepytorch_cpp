...
std::vector<std::vector<float>> DBPostProcessor::GetMiniBoxes(
...
std::vector<std::vector<std::vector<int>>> DBPostProcessor::BoxesFromBitmap(
  ...
  for (int _i = 0; _i < num_contours; _i++) {
    ...
    for (unsigned int num_pt = 0; num_pt < 4; num_pt++) {
      std::vector<int> a{
          int(clampf(
...
void PicodetPostProcessor::Run(std::vector<StructurePredictResult> &results,
  ...
  for (unsigned int i = 0; i < this->fpn_stride_.size(); ++i) {
  ...
  // for (unsigned int i = 0; i < bbox_results.size(); i++) {
...
StructurePredictResult PicodetPostProcessor::disPred2Bbox(
    std::vector<float> bbox_pred, int label, float score, int x, int y,
    int stride, std::vector<int> im_shape, int reg_max) {
...
void PicodetPostProcessor::nms(std::vector<StructurePredictResult> &input_boxes,
  ...
  for (unsigned int i = 0; i < input_boxes.size(); ++i) {
    ...
    for (unsigned int j = i + 1; j < input_boxes.size(); ++j) {
      if (picked[j] == 0) {
        continue;
      }
