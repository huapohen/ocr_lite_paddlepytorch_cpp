...
std::vector<std::string> Utility::ReadDict(const std::string &path) {
  ...
  if (in) {
    ...
    std::cout << "no such label file: " << path << ", error" << std::endl;
    // exit(1);
...
void Utility::VisualizeBboxes(const cv::Mat &srcimg,
  ...
  for (unsigned int n = 0; n < structure_result.cell_box.size(); n++) {
    if (structure_result.cell_box[n].size() == 8) {
      cv::Point rook_points[4];
      for (unsigned int m = 0; m < structure_result.cell_box[n].size();
           m += 2) {
        rook_points[m / 2] =
...
void Utility::sorted_boxes(std::vector<OCRPredictResult> &ocr_result) {
  ...
  if ((int)ocr_result.size() > 0) {
    for (unsigned int i = 0; i < ocr_result.size() - 1; i++) {
      for (int j = i; j > 0; j--) {
...
std::vector<float> Utility::activation_function_softmax(
